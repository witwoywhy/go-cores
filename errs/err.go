...
func NewInternalIntegrationError(errs ...error) Error {
	return &Errs{
		Errors: []Err{
			{
				Status:    http.StatusInternalServerError,
				ErrorCode: Err5009001,
				Err:       getError(errs...),
			},
		},
	}
...
func NewConflictError(code string, errs ...error) Error {
	return &Errs{
		Errors: []Err{
			{
